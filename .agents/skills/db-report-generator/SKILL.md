...
⛔ **Render `related_queries` của `seq_scan` bằng `<details>`, không phải bằng cách đoán cột filter** — mỗi row trong diagnostic `seq_scan` có thể có field `related_queries`, mỗi phần tử gắn `source` = `"window"` hoặc `"cumulative"`:
  - `source="window"` — query bắt được **trong sampling window** (có `window_calls` + `window_total_exec_time_ms`). Summary: `🔍 <code>{schema}.{table}</code> — {window_calls} lần gọi trong window`.
  - `source="cumulative"` — query lấy từ **pg_stat_statements tích luỹ** (fallback khi window không bắt được query nào chạm bảng; có `calls` + `total_exec_time_ms` LIFETIME kể từ lần pgss reset gần nhất, KHÔNG phải số trong window). Summary: `🔍 <code>{schema}.{table}</code> — {calls} lần gọi (lịch sử tích luỹ, từ khi pg_stat_statements reset)`. Ghi rõ trong `<details>` rằng đây là lịch sử, không phải quan sát trong lần chạy này.

  Với mọi phần tử, hiển thị nguyên văn query text trong khối `<details>` riêng cho bảng đó (convention Phần B: bảng chỉ có số liệu, query text nằm trong `<details>`), KHÔNG tự suy diễn cột filter qua tên bảng hay fix template mẫu. Khi `related_queries` rỗng (cả window lẫn cumulative đều không có query chạm bảng này), ghi rõ điều đó và gợi ý chạy lại report vào giờ cao điểm nghiệp vụ — thay vì im lặng bỏ qua.
