...
# {fn} is schema-qualified from caps so the call resolves on managed PG where the
# extension lives off the default search_path (Supabase/RDS).
_SQL = sql.SQL("""
SELECT ns.nspname AS schema, rel.relname AS tbl,
       st.table_len, st.dead_tuple_percent, st.approx_free_percent
FROM pg_class rel
JOIN pg_namespace ns ON ns.oid = rel.relnamespace
CROSS JOIN LATERAL {fn}(rel.oid) AS st
WHERE rel.relkind = 'r'
...
def collect(conn, caps):
    ext = (caps.get("extensions") or {}).get("pgstattuple")
    if not ext:
        return base.skipped(
            "table",
            "index_bloat requires the pgstattuple extension (not installed)")
    schema = ext.get("schema") or "public"
    query = _SQL.format(fn=sql.Identifier(schema, "pgstattuple_approx"))
